# MPNST TRAP1 Expression Analysis in Schwann Cell-derived Cancer
...
trap1_exp <- GetAssayData(schwann_cancer, layer = "data")["TRAP1",]
...
table_result <- table(schwann_cancer$TRAP1_status)
...
p1 <- FeaturePlot(schwann_cancer, features = "TRAP1") + 
  ggtitle("TRAP1 Expression in MPNST Schwann-derived Cancer") +
  theme(plot.title = element_text(size = 12, face = "bold"))

# Create TRAP1 status group UMAP
p2 <- DimPlot(schwann_cancer, group.by = "TRAP1_status", 
              cols = c("TRAP1-low" = "blue", "TRAP1-high" = "red")) +
  ggtitle("TRAP1 Status Groups") +
  theme(plot.title = element_text(size = 14, face = "bold"))

# Save visualizations
ggsave("pictures/TRAP1_expression.pdf", p1, width = 8, height = 7)
ggsave("pictures/TRAP1_status_groups.pdf", p2, width = 8, height = 7)
...
analyze_gene_category <- function(gene_list, category_name) {
  ...
  dot_plot <- DotPlot(schwann_cancer, 
                     ...
                     scale = FALSE) + 
            RotatedAxis() +
            ggtitle(paste(category_name, "Genes by TRAP1 Status")) +
            theme(plot.title = element_text(size = 14, face = "bold"),
                  axis.text.x = element_text(angle = 45, hjust = 1))
  
  # Save dotplot
  plot_width <- max(12, length(genes_present) * 0.4)  # Adjust width based on number of genes
  ggsave(paste0("pictures/TRAP1_", tolower(category_name), "_genes.pdf"), 
  ...
  for(gene in genes_present) {
    ...
    high_exp <- GetAssayData(schwann_cancer, layer = "data")[gene, schwann_cancer$TRAP1_status == "TRAP1-high"]
    low_exp <- GetAssayData(schwann_cancer, layer = "data")[gene, schwann_cancer$TRAP1_status == "TRAP1-low"]
  ...
  write.csv(gene_stats, 
           paste0("statistics/TRAP1_", tolower(category_name), "_genes_statistics.csv"), 
           row.names = FALSE)
...
if(length(all_top_genes) > 0) {
  ...
  p_top <- DotPlot(schwann_cancer, 
                  ...
                  scale = FALSE) + 
         RotatedAxis() +
         ggtitle("Top Significant Genes by TRAP1 Status") +
         theme(plot.title = element_text(size = 14, face = "bold"),
               axis.text.x = element_text(angle = 45, hjust = 1))
  
  # Save combined plot
  plot_width <- max(12, length(all_top_genes) * 0.4)
  ggsave("pictures/TRAP1_top_significant_genes.pdf", 
         p_top, width = plot_width, height = 7)
  
  # Create heatmap of top genes - avoid scaling warning by using normalized data
  # DoHeatmap does not strictly require scaled data
  p_heatmap <- DoHeatmap(schwann_cancer, 
                        features = all_top_genes, 
                        group.by = "TRAP1_status",
                        disp.min = -2.5, 
                        disp.max = 2.5) + 
              ggtitle("Top Significant Genes: TRAP1-high vs TRAP1-low")
  
  ggsave("pictures/TRAP1_top_genes_heatmap.pdf", 
